...
from nbxsync.models import ZabbixHostgroup, ZabbixHostgroupAssignment, ZabbixConfigurationGroup

__all__ = ('ZabbixHostgroupAssignmentForm', 'ZabbixHostgroupAssignmentFilterForm', 'ZabbixHostgroupAssignmentBulkEditForm')
logger = logging.getLogger(__name__)


class ZabbixHostgroupAssignmentForm(NetBoxModelForm):
    zabbixhostgroup = DynamicModelChoiceField(queryset=ZabbixHostgroup.objects.all(), required=True, selector=True, label=_('Zabbix Hostgroup'))
    ...
    fieldsets = (
        FieldSet('zabbixhostgroup', name=_('Generic')),
        FieldSet(
            TabbedGroups(
    ...
    def clean(self):
        ...
        if len(selected_objects) > 1:
            raise forms.ValidationError({selected_objects[1]: _('A Hostgroup can only be assigned to a single object.')})
...
class ZabbixHostgroupAssignmentFilterForm(NetBoxModelFilterSetForm):
    model = ZabbixHostgroupAssignment
    ...
    fieldsets = (
        ...
        FieldSet('zabbixhostgroup', name=_('Zabbix')),
    )

    tag = TagFilterField(model)


class ZabbixHostgroupAssignmentBulkEditForm(NetBoxModelBulkEditForm):
    model = ZabbixHostgroupAssignment
    zabbixhostgroup = DynamicModelChoiceField(queryset=ZabbixHostgroup.objects.all(), required=False, selector=True, label=_('Zabbix Hostgroup'))
    value = forms.CharField(label=_('Value'), max_length=2048, required=False)

    fieldsets = (FieldSet('zabbixhostgroup', 'value', name=_('Generic')),)
