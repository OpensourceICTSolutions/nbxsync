...
def get_primary_remote_device(interface):
    endpoints = list(interface.connected_endpoints or [])
    if not endpoints:
        return None

    remote_interface = endpoints[0]
    return getattr(remote_interface, 'device', None)
...
def get_connected_devices(device):
    ...
    for interface in device.interfaces.all().order_by('name'):
        remote_device = get_primary_remote_device(interface)
...
def sync_device_trigger_dependencies(device):
    ...
    if device_level_index < len(trigger_config.levels) - 1:
        result = _sync_child_dependency(device, trigger_config=trigger_config)
        if result:
            results.append(result)

    for child in get_child_devices(device, trigger_config=trigger_config):
        result = _sync_child_dependency(child, trigger_config=trigger_config)
        if result:
            results.append(result)
...
def _sync_child_dependency(child, trigger_config=None):
    trigger_config = trigger_config or get_plugin_settings().trigger_dependencies
    ...
    with ZabbixConnection(child_assignment.zabbixserver) as api:
