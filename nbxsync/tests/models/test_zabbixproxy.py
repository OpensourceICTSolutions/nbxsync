...
class ZabbixProxyTestCase(TestCase):
    ...
    def test_duplicate_allowed_addresses(self):
        proxy = ZabbixProxy(
            ...
            local_address='192.168.0.1',
            operating_mode=ZabbixProxyTypeChoices.ACTIVE,
            allowed_addresses=['10.0.0.1', '10.0.0.1'],
        )
        with self.assertRaises(ValidationError) as cm:
            proxy.full_clean()
        self.assertIn('allowed_addresses', cm.exception.message_dict)
    ...
    def test_tls_psk_validation(self):
        proxy = ZabbixProxy(
            ...
            tls_psk='abc123',  # Too short, should trigger validation error
            tls_psk_identity='',
        )
        with self.assertRaises(ValidationError) as cm:
            proxy.full_clean()
        ...
        self.assertIn('TLS PSK must be at least 32 characters long', error_msg)
        self.assertIn('TLS PSK Identity must be provided when using PSK encryption', error_msg)
    ...
    def test_invalid_local_address_format(self):
        proxy = ZabbixProxy(
            name='Bad Local',
            zabbixserver=self.zabbixserver,
            local_address='not@@valid',  # Invalid address format
            operating_mode=ZabbixProxyTypeChoices.ACTIVE,
        )
        with self.assertRaises(ValidationError) as cm:
            proxy.full_clean()
    ...
    def test_invalid_address_for_passive_proxy(self):
        proxy = ZabbixProxy(
            name='Invalid Passive Address',
            zabbixserver=self.zabbixserver,
            address='invalid@@',  # invalid format
            operating_mode=ZabbixProxyTypeChoices.PASSIVE,
        )
        with self.assertRaises(ValidationError) as cm:
            proxy.full_clean()
    ...
    def test_tls_psk_invalid_format(self):
        proxy = ZabbixProxy(
            ...
            operating_mode=ZabbixProxyTypeChoices.PASSIVE,
            tls_connect=ZabbixTLSChoices.PSK,
            tls_psk='!' * 32,  # Valid length, invalid hex
    ...
    def test_custom_timeout_not_a_string(self):
        proxy = ZabbixProxy(
            ...
            operating_mode=ZabbixProxyTypeChoices.ACTIVE,
            custom_timeouts=True,
            timeout_zabbix_agent=[30],  # Not a string; should raise the specific error
        )
        with self.assertRaises(ValidationError) as cm:
            proxy.full_clean()
        ...
        self.assertIn('timeout_zabbix_agent', cm.exception.message_dict)
        self.assertIn(
            "Invalid format: use integer + 's' or 'm', e.g. '90s', '5m'.",
            cm.exception.message_dict['timeout_zabbix_agent'][0],
        )
    ...
    def test_save_clears_tls_fields_based_on_tls_connect(self):
        proxy = ZabbixProxy(
            ...
            tls_psk='a' * 32,
            tls_psk_identity='identity123',
            tls_issuer='IssuerXYZ',
            tls_subject='SubjectXYZ',
        )
        proxy.full_clean()
        proxy.save()
        ...
        self.assertNotEqual(proxy.tls_psk, '')  # PSK should still be present
        self.assertNotEqual(proxy.tls_psk_identity, '')

        # Now test CERT path
        proxy = ZabbixProxy(
            name='TLS Cleanup Test - CERT',
            zabbixserver=self.zabbixserver,
            address='10.0.0.3',
            operating_mode=ZabbixProxyTypeChoices.PASSIVE,
    ...
    def test_get_tls_accept_display(self):
        proxy = ZabbixProxy.objects.create(
            ...
            operating_mode=ZabbixProxyTypeChoices.ACTIVE,
            tls_accept=[ZabbixTLSChoices.NO_ENCRYPTION, ZabbixTLSChoices.CERT],
        )
    ...
    def test_str_representation(self):
        ...
        expected_str = 'ProxyStrTest (Active)'
        self.assertEqual(str(proxy), expected_str)

    def test_active_with_group_requires_local_address(self):
        group = ZabbixProxyGroup.objects.create(
            zabbixserver=self.zabbixserver,
            name='Group 1',
            failover_delay=30,  # as suggested
            min_online=2,
        )

        proxy = ZabbixProxy(
            name='Active No Local',
            zabbixserver=self.zabbixserver,
            proxygroup=group,
            local_address='',  # missing on purpose
            operating_mode=ZabbixProxyTypeChoices.ACTIVE,
        )
