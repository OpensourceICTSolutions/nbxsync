...
@patch('nbxsync.views.hostinfo.ZabbixConnection')
def test_events_from_multiple_servers_are_all_kept(self, mock_conn):
    """
    Regression: `event_list = []` used to live inside the per-server loop, so
    the second server's events wiped the first's.
    """
    server_b = ZabbixServer.objects.create(name='ZBX-B', url='http://b.example', token='t')
    ZabbixServerAssignment.objects.create(zabbixserver=server_b, hostid=202, assigned_object_type=ContentType.objects.get_for_model(Device), assigned_object_id=self.device.pk)

    responses = {
        101: [_make_event('1', '1000')],  # from server A
        202: [_make_event('2', '2000')],  # from server B
    }

    def enter(instance):
        # Which server this connection is for — read from the arg the view passed.
        server = instance.call_args.args[0]
        api = MagicMock()
        api.event.get.side_effect = lambda **kw: [] if 'eventids' in kw else responses[ZabbixServerAssignment.objects.get(zabbixserver=server).hostid]
        return api

    mock_conn.return_value.__enter__.side_effect = lambda: enter(mock_conn)

    response = self.client.get(reverse('plugins:nbxsync:zabbixhost_events', kwargs={'objtype': 'device', 'pk': self.device.pk}))
    eventids = {r.record['eventid'] for r in response.context['table'].rows}
    self.assertEqual(eventids, {'1', '2'})


@patch('nbxsync.views.hostinfo.ZabbixConnection')
def test_unreachable_server_shows_in_fetch_errors(self, mock_conn):
    mock_conn.side_effect = ConnectionError('boom')

    response = self.client.get(reverse('plugins:nbxsync:zabbixhost_events', kwargs={'objtype': 'device', 'pk': self.device.pk}))
    self.assertEqual(response.status_code, 200)
    self.assertIn(self.server, response.context['fetch_errors'])
